...
--  limitations under the License.
  ...
  WITH S1 AS (
    ...
    T1 AS (
    SELECT ${keys}, max(recordstamp) as recordstamp from `${base_table}` where operation_flag in ('U', 'I') group by ${keys} order by recordstamp
    ),
    D1 AS (
    ...
    SELECT T1S1.* EXCEPT(recordstamp) from T1S1 where ${keys_comparator_with_t1s1_d1}
