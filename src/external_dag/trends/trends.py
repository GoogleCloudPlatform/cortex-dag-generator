...
def get_trends():
    try:
        pathlib = "/home/airflow/gcs/data/api_input/trends.ini"
        ...
        for k, v in ip.items():
            ...
            prodh = v.split(",")[1]
            end_date = date.today()
            timeframe = '{start_date} {end_date}'.format(
            start_date=start_date, end_date=end_date)
            ## CORTEX-CUSTOMER:After first run - can change to shorter rolling timeframe:
            ##  timeframe='today 7-d'            
            gprop = ""
            pytrend = TrendReq()
            pytrend.build_payload(kw_list=kw_list, geo=geo,
            ...
            interest_over_timedf=interest_over_timedf[['WeekStart','InterestOverTime','CountryCode','HierarchyId','HierarchyText']]    
            interest_over_timedf['WeekStart'] = pd.to_datetime(interest_over_timedf['WeekStart']).dt.strftime('%Y-%m-%d')
        ...
        table_schema= [ {'name': 'WeekStart', 'type': 'DATE' },
                { 'name': 'InterestOverTime', 'type': 'INTEGER' },
                { 'name': 'CountryCode','type': 'STRING' },
                { 'name': 'HierarchyId','type': 'STRING' },
                { 'name': 'HierarchyText', 'type': 'STRING' }]

        iot.to_gbq(target_trends_table,
                project_id=project_id_src, if_exists=write_mode, table_schema=table_schema)     
        print(iot.head())   
    except Exception:
        raise
