...
`{{ project_id_src }}.{{ dataset_cdc_processed }}.UpdateStockMonthlySnapshots`(
  start_date DATE,
  end_date DATE)
BEGIN
  ...
  FROM
    `{{ project_id_src }}.{{ dataset_cdc_processed }}.stock_monthly_snapshots`
  WHERE
  ...
  SELECT *
  FROM
    `{{ project_id_src }}.{{ dataset_cdc_processed }}.monthly_inventory_aggregation`
  WHERE
  ...
  WITH
    DateDim AS (
      SELECT DISTINCT
        ...
        PARSE_DATE('%Y%m%d', CAST(monthenddate AS STRING)) AS month_end_date
      FROM
