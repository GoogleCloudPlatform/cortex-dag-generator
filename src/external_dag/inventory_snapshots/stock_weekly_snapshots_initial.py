...
"""Cloud Composer DAG to generate initial stock weekly snapshot."""
...
with DAG(
    ...
    if AIRFLOW_VERSION.startswith("1."):
        t1 = BigQueryOperator(
            task_id='weekly_inventory_aggregation_sp',
            sql='0_weekly_inventory_aggregation.sql',
            create_disposition='WRITE_TRUNCATE',
            bigquery_conn_id='sap_cdc_bq',
            use_legacy_sql=False
        )
        t2 = BigQueryOperator(
            task_id='weekly_inventory_aggregation_update_sp',
            sql='stock_weekly_snapshots_inventory_aggregation_update.sql',
            create_disposition='WRITE_TRUNCATE',
            bigquery_conn_id='sap_cdc_bq',
            use_legacy_sql=False
        )
        t3 = BigQueryOperator(
            task_id='stock_weekly_snapshots_sp',
            sql='0_stock_weekly_snapshots.sql',
            create_disposition='WRITE_TRUNCATE',
            bigquery_conn_id='sap_cdc_bq',
            use_legacy_sql=False
        )
        t4 = BigQueryOperator(
            ...
            sql='stock_weekly_snapshots_initial.sql',
            create_disposition='WRITE_TRUNCATE',
            bigquery_conn_id='sap_cdc_bq',
            use_legacy_sql=False
        )
