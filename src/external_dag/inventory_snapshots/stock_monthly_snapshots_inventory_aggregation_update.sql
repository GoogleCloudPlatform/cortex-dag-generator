...
`{{ project_id_src }}.{{ dataset_cdc_processed }}.UpdateMonthlyInventoryAggregation`(
...
BEGIN
  ...
  SELECT
    ...
    src.meins,
    src.waers,
    PARSE_DATE('%Y%m%d', CAST(datedim.monthenddate AS STRING)) AS month_end_date,
    {% if sql_flavour == 'ecc' -%}
  ...
  LEFT JOIN
    ...
    WHERE
      src.budat_mkpf >= month_start_date_cut_off
     {% else -%}
      src.budat = datedim.Date
      WHERE
        src.budat >= month_start_date_cut_off
    {% endif -%}
    AND src.mandt = '{{ mandt }}'
  GROUP BY
    src.mandt,
