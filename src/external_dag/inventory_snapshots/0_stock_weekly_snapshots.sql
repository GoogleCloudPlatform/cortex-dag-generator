...
`{{ project_id_src }}.{{ dataset_cdc_processed }}.UpdateStockWeeklySnapshots`(
  start_date DATE,
  end_date DATE)
BEGIN
  ...
  FROM
    `{{ project_id_src }}.{{ dataset_cdc_processed }}.stock_weekly_snapshots`
  WHERE
  ...
  WITH
    DateDim AS (
      SELECT DISTINCT
        ...
        PARSE_DATE('%Y%m%d', CAST(weekEndDate AS STRING)) AS week_end_date,
        EXTRACT(YEAR FROM PARSE_DATE('%Y%m%d', CAST(weekEndDate AS STRING))) AS cal_year,
        EXTRACT(WEEK FROM PARSE_DATE('%Y%m%d', CAST(weekEndDate AS STRING))) AS cal_week
      FROM
        `{{ project_id_src }}.{{ dataset_cdc_processed }}.calendar_date_dim`
      WHERE
