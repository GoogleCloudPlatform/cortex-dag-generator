...
CREATE OR REPLACE VIEW `{{ project_id_src }}.{{ dataset_cdc_processed }}.ads_to_sap_prod_hier` AS
(
  ...
  (
    ...
    FROM `{{ project_id_src }}.{{ dataset_cdc_processed }}.ads_hier_mapping_sheet`
  )
  ...
  INNER JOIN `{{ project_id_src }}.{{ dataset_cdc_processed }}.prod_hierarchy` hier
