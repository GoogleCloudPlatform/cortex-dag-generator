...
CREATE OR REPLACE VIEW `{{ project_id_src }}.{{ dataset_cdc_processed }}.ads_to_sap_modified_mappings` AS
(
  WITH
  ordered_log AS
  (
    ...
    FROM `{{ project_id_src }}.{{ dataset_cdc_processed }}.ads_to_sap_prod_hier_log`
  ),
  ...
  combined_spreadsheet as (
    SELECT
      campaign_id, ad_group_id,
        STRING_AGG( CONCAT(IF(is_negative, "-|||", "+|||"), prodh), "|||" ORDER BY ad_group_id, prodh) as combined_mappings
    FROM `{{ project_id_src }}.{{ dataset_cdc_processed }}.ads_to_sap_prod_hier`
    GROUP BY campaign_id, ad_group_id
  )
