...
CREATE OR REPLACE VIEW
  `{{ project_id_tgt }}.{{ dataset_reporting_tgt }}.AdsDailyStatsByHierAndCountry`
AS
SELECT
  _DATA_DATE AS Date,
...
INNER JOIN `{{ project_id_src }}.{{ dataset_cdc_processed }}.ads_to_sap_prod_hier` mappings
  ON geo_stats.ad_group_id = mappings.ad_group_id
  AND mappings.is_negative IS FALSE
INNER JOIN `{{ project_id_src }}.{{ dataset_cdc_processed }}.ads_geo_targets` geo_targets
