...
CREATE OR REPLACE VIEW `{{ project_id_src }}.{{ dataset_cdc_processed }}.ads_targeting_criterions` AS
(
WITH
...
LEFT JOIN `{{ project_id_src }}.{{ dataset_cdc_processed }}.ads_affinity_categories` affinity
ON static_criterion_id = CAST(affinity.Criterion_ID AS STRING)
LEFT JOIN `{{ project_id_src }}.{{ dataset_cdc_processed }}.ads_in_market_categories` categories
ON static_criterion_id = CAST(categories.Criterion_ID AS STRING)
LEFT JOIN `{{ project_id_src }}.{{ dataset_cdc_processed }}.ads_verticals` verticals
ON static_criterion_id = CAST(verticals.Criterion_ID AS STRING)
LEFT JOIN `{{ project_id_src }}.{{ dataset_cdc_processed }}.ads_products_services` products
