...
echo "Checking Apache Beam Python dependencies..."
python3 -m pip install -U apache-beam[gcp]~={{ beam_sdk_version }} 1>/dev/null
...
then
    ...
    then
        ...
        deps_param="--requirements_file=./requirements.txt ${template_param}"
    fi
else
    echo "Checking pipeline's local Python dependencies..."
    python3 -m pip install -U -r "${SCRIPT_DIR}/catgap_pipeline/requirements.txt" 1>/dev/null
    deps_param="--requirements_file=./requirements.txt"
fi
...
python3 "${SCRIPT_DIR}/catgap_pipeline/main.py" \
    ...
    --sap_cdc_dataset "{{ dataset_cdc_processed }}" \
    --ads_dataset "{{ dataset_ads }}" \
    --ads_customer_id "{{ ads_account }}" \
    --staging_dataset "{{ dataset_cdc_processed }}" \
