...
def dag_gen_main(args: typing.Sequence[str]) -> int:
    ...
    config_paths = [
        ...
        # config subfolder in src/SAP/SAP_CDC
        # where CATGAP sits in src/SAP/SAP_CDC/src/external_dag/catgap
        # ^external_dag -> ^src -> ^SAP_CDC
        Path(_THIS_DIR).parent.parent.parent.joinpath("config",
                                                      _CONFIG_FILE_NAME),

        # config subfolder in Data Foundation root
        # where CATGAP sits in src/SAP/SAP_CDC/src/external_dag/catgap
        # ^external_dag -> ^src -> ^SAP_CDC -> ^SAP -> ^src > [root]
        Path(_THIS_DIR).parent.parent.parent.parent.parent.parent.joinpath(
            "config", _CONFIG_FILE_NAME),

        # config subfolder of the parent folder
        Path(_THIS_DIR).parent.joinpath("config", _CONFIG_FILE_NAME),
    ]
    ...
    reporting_dataset = sap_config["datasets"]["reporting"]
    mandt = sap_config["mandt"]

    # setting.yaml must be in config subfolder of the generator's folder
    ...
    dataset = bq_client.get_dataset(cdc_dataset)
    ...
    if options.skip_static_audiences is False:
        ...
        load_ads_audiences(bq_client, project_id, cdc_dataset)
    ...
    prepare_mapping_spreadsheet(mapping_url, project_id, cdc_dataset, location,
                                dataflow_sa, user_email)
