...
*/
CREATE OR REPLACE TABLE `{{ project_id_src }}.{{ dataset_cdc_processed }}.calendar_date_dim` AS
SELECT
  dt AS Date,
  ...
  CAST(FORMAT_DATE('%Y%m%d', (DATE_TRUNC(dt, WEEK))) AS INT64) AS WeekStartDate,
  CAST(FORMAT_DATE('%Y%m%d', (LAST_DAY(dt, WEEK))) AS INT64) AS WeekEndDate,
  CAST(FORMAT_DATE('%Y%m%d', (DATE_TRUNC(dt, MONTH))) AS INT64) AS MonthStartDate,
  CAST(FORMAT_DATE('%Y%m%d', (LAST_DAY(dt, MONTH))) AS INT64) AS MonthEndDate,
