...
CREATE OR REPLACE FUNCTION `{{ project_id_src }}.{{ dataset_cdc_processed }}.gdatu_to_date`(gdatu STRING) AS
(
  (SELECT PARSE_DATE('%Y%m%d', CAST(99999999 - CAST(gdatu AS INT) AS STRING)))
);

INSERT INTO `{{ project_id_src }}.{{ dataset_cdc_processed }}.currency_conversion`
WITH
  TCURR AS (
    ...
    SELECT
      ...
      `{{ dataset_cdc_processed }}`.gdatu_to_date(tcurr.gdatu) AS start_date,
      IF(
        LEAD(`{{ dataset_cdc_processed }}`.gdatu_to_date(tcurr.gdatu)) OVER (
  ...
  TCURF AS (
    ...
    SELECT
      ...
      tcurf.tcurr,
      tcurf.gdatu,
      `{{ dataset_cdc_processed }}`.gdatu_to_date(tcurf.gdatu) AS start_date,
      IF(
        LEAD(`{{ dataset_cdc_processed }}`.gdatu_to_date(tcurf.gdatu)) OVER (
